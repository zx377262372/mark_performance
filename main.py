...
async def process_summoner(summoner_name, riot_api, game_analyzer, prompt_generator, ai_analyzer, wechat_sender, logger):
    """处理单个召唤师的对局分析"""
    try:
        ...
        logger.error(f"处理召唤师 {summoner_name} 时出错: {str(e)}")
                ...
                for summoner_name in Config.SUMMONER_NAMES:
                    if summoner_name.strip():
                        task = asyncio.create_task(
                            process_summoner(
                                summoner_name, riot_api, game_analyzer, prompt_generator, 
                                ai_analyzer, wechat_sender, logger
                            )
                        )
                        tasks.append(task)
